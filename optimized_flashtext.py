...
class OptimizedFlashText(object):
    """KeywordProcessor
    ...
    def __init__(self, input_keys, case_sensitive=False):
        """
        Args:
        ...
        if not case_sensitive:
            input_keys = [x.lower() for x in input_keys]
        self.keyword_trie_dict = dawg.CompletionDAWG(input_keys)
    ...
    def extract(self, sentence, span_info=False, max_cost=0):
        ...
        while idx < sentence_len:
            char = sentence[idx]
            running_word = sentence[sequence_start_pos:idx]
            # when we reach a character that might denote word end
            if char not in self.non_word_boundaries:
                temp = idx
                # if end is present in current_dict
                # if self._keyword in current_dict or char in current_dict:
                has_key = self.keyword_trie_dict.has_key(running_word)
                if has_key or self.keyword_trie_dict.has_keys_with_prefix(running_word):
                    ...
                    running_word = sentence[sequence_start_pos:idx]
                    if has_key:
                        sequence_found = running_word
                        longest_sequence_found = running_word
                        sequence_end_pos = idx
                        keywords_extracted.append(
                            (running_word, sequence_start_pos, idx))

                    # re look for longest_sequence from this position
                    if self.keyword_trie_dict.has_keys_with_prefix(sentence[sequence_start_pos:(idx + 1)]):
                        # current_dict_continued = current_dict[char]

                        idy = idx + 1
                        while idy < sentence_len:
                            inner_char = sentence[idy]
                            current_word = sentence[sequence_start_pos:idy]
                            if inner_char not in self.non_word_boundaries and self.keyword_trie_dict.has_key(current_word):
                                # update longest sequence found
                                longest_sequence_found = current_word
                                sequence_end_pos = idy
                                is_longer_seq_found = True
                                keywords_extracted.append(
                                    (current_word, sequence_start_pos, idx))

                            if self.keyword_trie_dict.has_keys_with_prefix(current_word):
                            ...
                            if self.keyword_trie_dict.has_key(sentence[sequence_start_pos:]):
                                # update longest sequence found
                                longest_sequence_found = sentence[sequence_start_pos:]
                                sequence_end_pos = idy
                                is_longer_seq_found = True
                    ...
                    current_dict = self.keyword_trie_dict
                    reset_current_dict = True
            elif self.keyword_trie_dict.has_keys_with_prefix(running_word):
            ...
            if idx + 1 >= sentence_len:
                ...
                if sentence[sequence_start_pos:] in self.keyword_trie_dict:
                    sequence_found = sentence[sequence_start_pos:]
    ...
    def replace_keywords(self, sentence, max_cost=0):
        ...
        while idx < sentence_len:
            ...
            if char not in self.non_word_boundaries:
                        ...
                        while idy < sentence_len:
                            ...
                            if inner_char in current_dict_continued:
                                ...
                                current_dict_continued, cost, _ = next(
                                    self.levensthein(
                                        next_word, max_cost=curr_cost, start_node=current_dict_continued),
                                    ({}, 0, 0)
                                )
                                idy += len(next_word) - 1
                                curr_cost -= cost
                ...
                next_word = next_orig_word if self.case_sensitive else str.lower(
                    next_orig_word)
    ...
    # TODO: Need new levensthein distance that also functions as generator but does not have this type of
    # sub dict, rather uses iterator and passes words in accordingly assuming they
